...
author: "Esmeralda Vargas Madrigal<br>Jesús Monge Mejia<br>José Miguel Zeledón Calderón"
date: "2020-10-07"
output:
  html_document:
    theme: readable    
...
### Ejercicio 6 (15%) -Nota: Condición lógica-
...
```{r}
# Se extraen las observaciones correspondientes a los países de Asia
Paises_Asia = world[world$continent == "Asia" & world$lifeExp >= 80, ]
Paises_Asia2 = na.omit(Paises_Asia)
ggplot(Paises_Asia2, aes(x=reorder(name_long, -lifeExp), y=lifeExp)) +
  ggtitle("Paises Asiaticos que superan la esperanza de vida mayor a 80 años") +
  xlab("País") + 
  ylab("Esperanza de Vida (Años)") +  
  geom_col() +
  geom_text(aes(label=(format(round(lifeExp,3)))), vjust=-0.3, size=2.5, )
```

### Ejercicio 7 (20%) -Filtrar la columna de continente-
...
```{r}
# Países del continente Oceanía
oceania_paises = world[world$continent == "Oceania", ]

# Mapa de esperanza de vida de Oceanía
plot(
  oceania_paises["lifeExp"], 
  main="Esperanza de Vida de los Países de Oceanía"
)
```
...
```{r}
# Filtro para crear data frame con países de Centroamérica con esperanza de vida mayor a 74 años
paises_ca = world[world$subregion == "Central America" & world$lifeExp >= 74, ]

plot(
  paises_ca["lifeExp"] , 
  main="Esperanza de Vida mayor a 74 años de los Países de Centroamérica"
)
```

### Ejercicio 9 (20%) - 2 mapas de poropletas 1) con el área 2)población-
...
```{r}
paises_sa <- world[world$subregion == "South America",] %>%
  filter(pop >= 40000000 | area_km2 >= 2500000)

ggplot(paises_sa, aes(fill=pop))+
  geom_sf()+
  geom_sf_label(aes(label=name_long))+
  scale_fill_gradient2(low = "yellow", mid= "orange", high = "blue", name = "Habitantes", labels = comma)+
  ggtitle("Países de América del Sur con una población mayor o igual a 40 millones de personas")+
  xlab("")+ylab("")+
  theme_bw()+
  theme(title =element_text(size=10, face='bold'))

paises_sa %>%
  filter(name_long != "Colombia") %>%
  ggplot(aes(fill=area_km2))+
  geom_sf()+
  geom_sf_label(aes(label=name_long))+
  scale_fill_gradient2(low = "orange", mid= "yellow", high = "green", name = "km2", labels = comma)+
  ggtitle("Países de América del Sur con un área de al menos 2,5 millones de km2")+
  xlab("")+ylab("")+
  theme_bw()+
  theme(title =element_text(size=10, face='bold'))
```